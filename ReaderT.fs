...
type ReaderT<'R,'Ma> with
    ...
    static member inline (?<-) (_MonadReader:Ask, _:ReaderT<'r,'a>,         _) = ReaderT return' :ReaderT<'r,'a>
    static member inline (?<-) (_MonadReader:Local, ReaderT m, _:ReaderT<_,_>) = fun f -> ReaderT(fun r -> m (f r))

    static member inline (?<-) (_MonadIO:LiftIO,  _:ReaderT< 'r, 'm_a> , _) = fun (x: IO< 'a>) -> ((lift ((liftIO x) : 'ma2)) : 'R2)
    ...
    static member (?<-) (_MonadState:Get    , _:ReaderT<'s,State<'a,'a>>  , _) = lift    get :       ReaderT<'s,State<'a,'a>>
    ...
    static member (?<-) (_MonadWriter:Listen, ReaderT m, _:ReaderT<'t,Writer<'a,'b*'a>>) :ReaderT<'t,Writer<'a,'b*'a>>    = ReaderT <| fun w -> listen (m w)  
    static member (?<-) (_MonadWriter:Pass  , ReaderT m, _:ReaderT<'t,Writer<'a,'b>>)    :ReaderT<'t,Writer<'a,'b>>       = ReaderT <| fun w -> pass   (m w)  
