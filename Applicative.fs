...
type Applicative = Applicative with
    member inline this.pure'                        x = return' x
    static member pure' (Applicative, _:'a option   ) = fun (x:'a) -> Applicative.pure' x :'a option
    static member pure' (Applicative, _:'a list     ) = fun (x:'a) -> Applicative.pure' x :'a list
    static member pure' (Applicative, _:'a IO       ) = fun (x:'a) -> Applicative.pure' x :'a IO
    static member pure' (Applicative, _: _ -> 'a    ) = const'
    static member pure' (Applicative, _:Either<'e,_>) = fun (x:'a) -> Applicative.pure' x :Either<'e,_>

    member inline this.ap f = fun x -> ap f x
    static member ap (Applicative, f:option<_>   , x:option<_>   ) = Applicative.ap f x
    static member ap (Applicative, f:list<_>     , x:list<_>     ) = Applicative.ap f x
    static member ap (Applicative, f:IO<_>       , x             ) = Applicative.ap f x
