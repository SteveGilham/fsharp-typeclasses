...
module List =
    type ListT< ^ma > = ListT of (^ma ) with
        static member inline (?<-) (_:Return, cs:Return, t:ListT<_>) = ListT << return' << singleton
...
type Lift = Lift with
    static member inline (?<-) (x, cs:Lift, t:Maybe.MaybeT<_>)  = Maybe.MaybeT << (liftM Some)      <| x
    static member inline (?<-) (x, cs:Lift, t:List . ListT<_>)  = List .ListT  << (liftM singleton) <| x
