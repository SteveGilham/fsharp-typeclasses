...
module MaybeT =
    ...
    type MaybeT< ^ma > with
        static member inline return' (Monad, t:MaybeT<_>  ) = MaybeT << return' << Some
        ...
        static member inline mzero (MonadPlus, _:MaybeT<_>  ) = MaybeT (return' None)
...
type MonadCont = MonadCont with
    static member inline callCC (MonadCont, f, _:MaybeT<_>) = MaybeT(callCC <| fun c -> runMaybeT(f (MaybeT << c << Some)))    
...
type MonadWriter = MonadWriter with
    ...
    static member inline pass (MonadWriter, m, _:MaybeT<_>  ) = MaybeT (runMaybeT m >>= maybe (return' None) (liftM Some << pass << return'))
