...
type Foldr = Foldr with
    static member (?<-) (_Foldable:Foldr, x:Maybe<_>, _) = fun (f,z) -> match x with |Just t -> f t z |Nothing -> z
    static member (?<-) (_Foldable:Foldr, x:List<_> , _) = fun (f,z) -> List.foldBack           f x z
...
type Foldr with
    static member inline (?<-) (_Foldable:Foldr, x:array<_>, _) = fun (f,z) -> Foldable.foldr f z x

type Foldl = Foldl with
    static member (?<-) (_Foldable:Foldl, x:Maybe<_>, _) = fun (f,z) -> match x with |Just t -> f z t |Nothing -> z
