...
type StateT<'S,'MaS> with
    ...
    static member inline (?<-) (_MonadPlus:Mzero, _:StateT<_,_>,     _) = fun ()         -> StateT <| fun _ -> mzero()
    static member inline (?<-) (_MonadPlus:Mplus,   StateT m,        _) = fun (StateT n) -> StateT <| fun s -> mplus (m s) (n s)

    static member inline (?<-) (_MonadTrans:Lift, _:StateT<'s,'mas>, _) = fun (m:'ma) -> (StateT <| fun s -> m >>= fun a -> return' (a,s) ):StateT<'s,'mas>
    
    static member inline (?<-) (_MonadState :Get, _:StateT<_,_>,     _) = fun () -> StateT (fun s -> return' (s , s))
    static member inline (?<-) (_MonadState :Put, _:StateT<_,_>,     _) = fun x  -> StateT (fun _ -> return' ((), x))

    static member inline (?<-) (_MonadIO: LiftIO, _:StateT<_,_>,     _) = fun (x: IO<_>) -> lift (liftIO x)
