...
type StateT< ^s, ^m> = StateT of (^s -> ^m) with
    static member inline (?<-) (_:unit  , _Functor:Fmap, StateT m       ) = fun f -> StateT <| fun s -> do'{
        let! (x, s') = m s
        return (f x, s')}

    static member inline (?<-) (_:unit  , _Monad  :Return, _:StateT<_,_>) = fun a -> StateT (fun s -> return' (a, s))
    ...
    static member inline (?<-) (_:unit  , _MonadPlus:Mzero, _:StateT<_,_>) = StateT <| fun _ -> mzero()
    ...
    static member inline (?<-) (_:unit  , _MonadState:Get, _:StateT<_,_> ) =          StateT (fun s -> return' (s , s))
    static member inline (?<-) (_:unit  , _MonadState:Put, _:StateT<_,_> ) = fun x -> StateT (fun _ -> return' ((), x))
