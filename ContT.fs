...
type ContT<'Mr,'A> with
    ...
    static member inline (?<-) (_MonadIO:LiftIO   , _:ContT<_,_>,    _) = fun (x: IO<_>) -> lift (liftIO x)

    static member (?<-) (_MonadReader:Ask  , _:ContT<Reader<'a,'b>,'a>, _) = fun () -> lift ask :ContT<Reader<'a,'b>,'a>
    ...
    static member (?<-) (_MonadState:Get   , _:ContT<State<'s,'a>,'s>  , _) = fun () -> lift get:ContT<State<'s,'a>,'s>
